// Live Stream page — data. Fictional / original comic content.
window.STREAM_DATA = (function () {
  const host = {
    name: 'thundervault',
    rating: 4.9,
    viewers: 62,
  ...
  // The item currently on the block (just sold, awaiting next)
  const current = {
  ...
  // Shop products for sale in this show
  const products = [
  ...
  // Chat — `q` flags a buyer question (rendered in the brand accent like the ref)
  const chat = [
  ...
  return { host, current, giveaway, products, upcoming, chat };
})();
