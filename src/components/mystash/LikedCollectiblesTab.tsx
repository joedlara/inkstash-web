...
export default function LikedCollectiblesTab() {
  ...
  const loadLikedItems = async () => {
    ...
    try {
      ...
      setError('Failed to load liked collectibles');
    } finally {
      setLoading(false);
    }
  };

  const handleUnlike = async (likeId: string, auctionId: string) => {
    try {
      ...
      setError('Failed to remove liked item');
    }
  };
    ...
    <Box>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          {likedItems.length} {likedItems.length === 1 ? 'item' : 'items'} you've favorited
        </Typography>
      </Box>
      ...
      {likedItems.length === 0 ? (
        ...
        >
          ...
          <Typography variant="h5" gutterBottom fontWeight={600}>
            No Liked Collectibles
          </Typography>
          <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
            You haven't liked any items yet. Browse collectibles and like your favorites!
          </Typography>
      ...
      >
        <MenuItem
          onClick={() => {
            const item = likedItems.find(i => i.id === selectedItem);
            if (item) {
              handleUnlike(item.id, item.auctions.id);
            }
          }}
        >
